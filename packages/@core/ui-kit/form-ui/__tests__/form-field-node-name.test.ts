...
// Shared form behavior must not depend on an optional application's UI library
// or a version-specific pnpm store path. Native controls expose the name clash.
...
const ProbeTreeSelect = defineComponent({
  ...
  setup(props, { attrs }) {
    ...
    return () => h(NativeSelect, { ...props, ...attrs });
  },
});
...
describe('issue #8214: form field named nodeName crashes TreeSelect render', () => {
  it('renders MRE schema (TreeSelect parentId + Input nodeName) without error', async () => {
    ...
    // Shared regression: interacting with a control must preserve form.nodeName.
